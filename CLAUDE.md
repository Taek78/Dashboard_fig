...
- Messages (`/messages`) : boîte de réception des demandes « Nous contacter », **écrites par l'application FIG** ; le dashboard ne pose que trois marques, toutes en écriture conditionnelle sur l'état relu (`status` non traité / traité, `pinned_at` épinglé en tête, `important`), réservées à l'admin et au gestionnaire (`canHandleMessages`). Objet parmi six valeurs, commande associée facultative, dix pièces jointes au plus : la limite et la liste blanche de formats (PDF et images, ni vidéo ni audio) sont tenues par la BASE (enum `attachment_content_type`, `position` bornée), pas par un écran, puisque l'application écrira directement. Les fichiers sont hébergés par l'application (question 22) : le dashboard n'en stocke que les métadonnées et l'URL. Liste triée épinglés d'abord puis les plus récents, carte avec les deux premières lignes (`messagePreview`), fiche complète précédée de la `CustomerCard` du client.
