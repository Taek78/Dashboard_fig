...
- Domaines existants : `orders`, `deliveries` (règles pures de tournée seulement : l'attribution de livreur a été retirée le 2026-09-13), `products`, `customers`, `metrics` (agrégations pures : TVA HT/TTC, périodes, séries, comparaison N-1 ou période précédente), `engagement` (usage de l'appli par mois : téléchargements, inscriptions, réclamations, note), `auth` (rôles, `UserAccount`, `loginSchema`). Chaque domaine métier a sa façade `src/data/<domaine>.ts` qui appelle `selectSource(domaine, mock, db | null)` (B1) : `DATA_SOURCE=db` sans implémentation Drizzle lève une erreur explicite.
