...
- **Tailwind v4** via `@tailwindcss/postcss`, thème dans `src/app/globals.css` (`@theme inline`) : trois modes par `data-theme` (light, dark, fig), tokens oklch, `--success`, `--warning`, `--info`, `--fruit`, `--vegetable`, `--halo-1/2`, dégradé de marque ; le variant `dark:` couvre dark et fig. Jamais de couleur en dur. Utilitaires maison : `bg-gradient-brand`, `text-gradient-brand`, `card-lift` (élévation au survol), `glow-brand` (halo de coin), `skeleton-shimmer`, `page-in` (entrée de page dans `(dashboard)/template.tsx`), `float-soft` ; bouton `variant="brand"` pour l'action principale d'un écran ; bordure gauche des cartes de commandes colorée par statut (`STATUS_ACCENT`).
...
- Logique pure isolée et testée ; les composants ne font que brancher.
