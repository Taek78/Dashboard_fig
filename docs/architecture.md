...
Un dossier par domaine : `orders` (commandes, avec `assignment.ts` pour l'affectation de l'équipe, `discount.ts` pour les remises et la meilleure remise, `delivery-fee.ts` pour le barème des frais de livraison, `slot.ts` pour le créneau d'une heure), `deliveries` (tournée), `products` (catalogue), `customers` (clients, avec `loyalty.ts` pour le compteur de fidélité, `tier.ts` pour la catégorie basique ou fidèle et son historique, `consents.ts` pour les trois autorisations, `referral.ts` pour le code de parrainage, `client-type.ts` pour particulier ou communauté, `directory.ts` pour la recherche commune et ses tris dans les deux sens), `communities` (groupes de clients livrés à un même point de retrait, `discount.ts` pour la remise selon le nombre de membres), `notifications` (file des notifications d'état déposées pour les clients : texte, contrat, fixtures), `staff` (personnel : livreurs, préparateurs, gestionnaires), `articles`, `metrics` (agrégations), `engagement` (usage de l'appli), `auth` (rôles et comptes), `messages` (boîte de réception « Nous contacter » : objets, statut, pièces jointes, aperçu et tri), `privacy` (RGPD : durées de conservation, anonymisation, export des données d'un client ; voir [rgpd.md](rgpd.md)). Chaque dossier suit le même patron :
