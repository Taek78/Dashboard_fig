...
| `notifications/[id]/route.ts`                   | l'état d'envoi d'une notification déposée (en attente, envoyée, en échec), relu toutes les 3 s par le badge « Client notifié » ; qui change un statut seulement, autre site refusé, `no-store`, ne renouvelle pas la session |
| `alertes/actions.ts`                            | `markSectionSeen` (dernière visite d'une section : son compteur non lu repart de zéro) et `saveAlertPrefs` (cases de « Mon profil »), pour le compte de la session seulement |
