...
**Modifications non enregistrées** (formulaires, 2026-09-18) : l'avertissement qui s'ouvre quand on quitte la fiche d'un membre de l'équipe, d'un produit ou la note d'un client après l'avoir modifiée sans enregistrer : « Enregistrer les modifications » ou « Quitter ». Fermer l'onglet déclenche la boîte du navigateur (*beforeunload*).

**Badge « Nouveau »** (cartes, 2026-09-18) : la pastille qui scintille sur la carte d'une commande ou d'un message arrivé pendant la session, avec une légère surbrillance de la carte. Elle dure une minute, s'éteint à l'ouverture du détail, et au rechargement de la page seulement si l'on est entré dans la section depuis. Elle reste active même si les notifications de ce type sont désactivées dans « Mon profil ».

**Compteur non lu** (menu, 2026-09-18) : le nombre à côté de Commandes, Messages et Catalogue : commandes créées, messages reçus, produits passés en stock critique ou à 0 depuis la DERNIÈRE VISITE de la section, enregistrée sur le compte (il survit au rechargement et suit la personne d'un poste à l'autre). Cliquer sur la section le remet à zéro. « Mon profil » permet de couper les notifications de commandes ou de messages (bandeau, son, notification système) et le son seul ; le compteur, lui, reste toujours actif.
...
**Notification groupée** (alertes en direct) : une seule notification s'affiche à la fois ; ce qui arrive pendant qu'elle est visible s'y ajoute et relance ses 10 s (« 3 nouvelles commandes », « 2 commandes · 1 message »), au lieu d'empiler des cartes (`summarizeNotices`).

**Alerte en direct** (écrans, 2026-09-18) : la notification qui descend de sous le bandeau pendant 10 s quand une commande arrive, qu'un client écrit, ou qu'un produit passe en stock critique ou à 0. Le navigateur **relève** (*polling*) le flux `GET /alertes` toutes les 5 s et le compare au relevé précédent : seul ce qui est nouveau depuis l'ouverture de la page est annoncé. Une nouvelle commande sonne.
...
**Remboursement / avoir** (commandes, 2026-09-19) : ce que l'équipe enregistre sur une commande ANNULÉE. Un **remboursement** rend l'argent au client ; un **avoir** est un crédit à valoir sur une prochaine commande. Total ou partiel, jamais au-dessus du total payé. Tant qu'il existe, la commande reste annulée (« Statut verrouillé »). Badge plein cuivre (remboursement) ou sarcelle (avoir), carte de la même couleur ; métrique en % de toutes les commandes de la période.
