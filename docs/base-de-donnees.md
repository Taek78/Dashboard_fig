...
| `communities`        | groupes de clients livrés à un point de retrait, livraison offerte ; pas de taux de remise stocké (déduit du nombre de membres, colonne supprimée en 0009) ; pas d'heure de retrait (choisie à chaque commande, colonne supprimée en 0003) | créées par l'application FIG, lues par le dashboard                                                                                                                                                                      |
| `customers`          | clients de l'application ; `address_line` (rue), `community_id` pour les membres ; autorisations `notify_offers`, `notify_order_status`, `marketing_consent` datées par `consents_updated_at` ; parrainage `referral_code` (« Nom#0000 », unique) et `referred_by_id` (clé sur la table elle-même) ; `anonymized_at` (migration 0007) | e-mail unique sans casse ; code au format vérifié par la base, un client ne se parraine pas lui-même ; RGPD : données personnelles, jamais supprimées (commandes) mais anonymisées (`src/db/privacy.ts`) ; communauté ou parrain supprimé → `SET NULL` |
...
| `orders`             | commandes : statut, client, créneau d'une heure, rue de livraison (`delivery_address_line`, instantané), frais de livraison (`delivery_fee_cents`), montant dû, motif d'annulation, communauté et remise (`discount_kind`, `discount_percent`, `discount_cents`), préparateur et livreur (`preparer_id`, `driver_id`) | référence unique ; un client avec des commandes ne peut pas être supprimé ; motif présent si et seulement si annulée ; remise cohérente (nature et pourcentage ensemble) ; créneau d'une heure pile (`orders_slot_one_hour`) ; frais ≥ 0 et nuls pour une communauté (`orders_community_delivery_free`) ; personne ou communauté supprimée → `SET NULL` |
...
| `customer_notifications` | file des notifications d'état déposées pour les clients : commande, statut annoncé, titre, texte, dépôt, envoi (`sent_at`)                                                                         | écrite par `updateOrderStatus` dans la transaction du statut, seulement si `customers.notify_order_status` ; l'application FIG lit les lignes sans `sent_at` (index partiel) et pose l'envoi ; supprimées à l'anonymisation du client |
...
| `engagement_monthly` | usage de l'appli par mois (téléchargements, inscriptions, note) ; plus de colonne « réclamations » (supprimée en 0010 : elles se comptent dans `customer_messages`)                                        | `rating` en `numeric(3,2)`, converti en nombre par le mapper                                                                                                                                                             |
...
Listes de valeurs : enums Postgres (`order_status`, `cancellation_reason`, `product_category`, `product_unit`, `container`, `article_category`, `user_role`, `staff_kind`, `staff_shift`, `staff_availability`, `community_kind`, `discount_kind`, `message_subject`, `message_status`, `attachment_content_type`, `notification_kind`), identiques aux constantes du domaine (vérifié par `test/db/schema.test.ts`). Ajouter une valeur = modifier le domaine ET le schéma, puis `db:generate`. La migration 0006 commence par deux instructions écrites à la main que drizzle-kit ne génère pas : `CREATE EXTENSION IF NOT EXISTS pg_trgm` (extension « de confiance » : le rôle qui migre doit seulement avoir le droit CREATE sur la base, à vérifier sur la base du client) et la fonction `fig_normalize`. Retirer une valeur (comme `confirmed` en 0003, puis `pending` en 0005, qui retire aussi le temps de la reprise la contrainte `orders_cancellation_consistent`) exige une migration des données écrite à la main dans le SQL généré, avant la recréation du type. La migration 0009 contient de même une instruction écrite à la main : la fin des créneaux existants est réécrite à « début + une heure » avant de poser la contrainte `orders_slot_one_hour` ; elle supprime aussi la colonne `communities.discount_percent`.
...
- la liste des commandes est paginée par la base : identifiants de la page d'abord, puis jointures et lignes (JSON) pour ces commandes seulement, total compté en parallèle ; tableau de bord, métriques, personnel, fiches et annuaire lisent des **agrégats** (`count(*) filter (where …)`, `GROUP BY` du premier jour du seau, `min()` d'un tableau pour le nom d'un produit sans tout trier, `cross join lateral` pour compter le travail d'une personne sans jointure OR, des **fonctions de fenêtre** pour la fidélité : un compteur cumulé des commandes remisées numérote les cycles de chaque client, `row_number()` numérote les commandes comptées dans leur cycle, la huitième date l'atteinte de la catégorie « fidèle ») dans `orders-aggregates.db.ts`, sans charger l'historique ;
- `updateOrderStatus` est une mise à jour conditionnelle (`WHERE id = $1 AND status = $2`) dans une transaction avec l'insertion de l'événement, puis le dépôt de la notification par un `INSERT … SELECT … WHERE notify_order_status` (le consentement est relu par la base au moment d'écrire) : deux personnes ne peuvent pas écraser le même statut, et rien n'est déposé pour un client qui n'a pas donné son accord ;
