...
- **Qualité** : 494 tests Vitest, 15 tests de contrat mock = PostgreSQL, 33 parcours Playwright (dont tablette 768 px et menu replié), CI (check, audit, Playwright en mock puis contre PostgreSQL).
...
- Agrégations (métriques, compteurs du personnel, annuaire clients) et pagination des commandes en SQL : aujourd'hui toutes les commandes sont chargées puis calculées en mémoire, suffisant pour les volumes de démonstration, premier goulot quand l'historique réel grossira.
...
- Après audit : affectation conditionnelle (commande non terminée, affectation inchangée depuis l'affichage), limitation de débit en base (`login_attempts`, migration 0004), tests de contrat mock = Postgres en CI. Agrégations et pagination en SQL reportées à la connaissance des volumes réels (2026-09-15).
