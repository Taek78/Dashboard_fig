...
- **Sécurité, suites de l'audit du 2026-09-14** : faits le jour même : CSP avec nonce, journal de sécurité en table `security_events`, gestion des comptes, sauvegardes. Restent : `serverActions.allowedOrigins` à régler au déploiement derrière un proxy inverse ; limitation de débit partagée (base ou cache) si plusieurs instances ; TLS vers Postgres et rôle applicatif à privilèges réduits (mise en ligne) ; rotation/révocation de session côté serveur ; rôle « livreur » et matrice `SECTION_ACCESS` à confirmer avec le client (Q5).
