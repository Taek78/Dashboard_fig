...
| B2 ✅ (revu) | **Conception du schéma** (2026-09-14 : le client demande au dashboard de créer la base) | `src/db/schema.ts` (9 tables, 7 enums, contraintes), migration `drizzle/0000_base-fig.sql`, `scripts/setup-local.sql`, `scripts/seed.ts`, `docs/base-de-donnees.md` | `npm run db:migrate` puis `db:seed` sur la base locale, `db:studio` montre les données | création locale de la base (mot de passe postgres de Zaki) |
| B3 ✅ | Mapping et implémentation Drizzle par domaine | `src/data/<domaine>.db.ts` pour commandes, catalogue, clients, articles, usage, comptes ; `src/db/mappers.ts` purs testés en aller-retour sur les fixtures | `npm run check` vert avec les deux implémentations | B1 |
| B4 ✅ | Bascule `DATA_SOURCE=db` en local (2026-09-14) | Tous les écrans et les 11 tests navigateur vérifiés contre la base locale seedée | Playwright vert en mode db | — |
| B5 | Branchement de l'application FIG | Comment l'appli du client lit et écrit cette base (API du dashboard ? accès direct ?) : à décider avec le client | Une commande passée dans l'appli apparaît dans le dashboard | question client (Q14) |
...
14. **Branchement de l'application FIG à la base** (2026-09-14) : la base est créée par le dashboard ; l'application y accède comment ? API HTTP exposée par le dashboard (recommandé : un seul propriétaire du schéma) ou accès SQL direct avec un rôle dédié ? Qui héberge Postgres ? Volumes attendus ?
...
- **Sécurité, suites de l'audit du 2026-09-14** : CSP avec nonce (le script anti-flash du thème et recharts imposent aujourd'hui `'unsafe-inline'`) ; `serverActions.allowedOrigins` à régler au déploiement derrière un proxy inverse ; limitation de débit partagée (base ou cache) si plusieurs instances ; journal de sécurité en base (B2) ; TLS vers Postgres et utilisateur de base à privilèges réduits (B2) ; retirer `AUTH_ALLOW_BOOTSTRAP` dès que les comptes viennent d'une base ; rotation/révocation de session côté serveur ; rôle « livreur » à confirmer avec le client (Q5), matrice `SECTION_ACCESS` à ajuster.

- Filtres, tri, pagination, page de détail, lien sur les lignes (A2).
- Livraisons revues le 2026-09-13 : **attribution de livreur retirée** à la demande du client (livreurs, attributions, action `assignCourier`, règle de conflit et fixtures supprimés ; `canAssignCourier` retiré des rôles). La tournée se pilote par le statut des commandes : cartes horizontales (créneau, client, contenu, changement de statut), compteurs à confirmer / en cours / terminées. Sidebar repliée sans texte résiduel, bouton de menu arrondi, tableaux défilables horizontalement sur mobile.
- Indicateurs de tendance ajoutés le 2026-09-13 : badge fléché vert / rouge (pourcentage vs N-1 ou vs période précédente, au choix), lecture inversée pour les annulations et réclamations ; bloc « usage de l'application » sur l'année civile (téléchargements, inscrits / téléchargements, inscrits ayant commandé, réclamations, note) sur un nouveau domaine `engagement` avec fixtures mensuelles déterministes 2024 → 2026. Question client 11 : source et fréquence de ces chiffres (stores, support).
- Métriques étendues le 2026-09-13 : montants HT / TTC (TVA 5,5 % déduite des TTC, taux et base à confirmer avec le client, Q9), périodes prédéfinies (aujourd'hui, hier, semaine dernière, ce mois, mois dernier, 90 jours, 6 mois, cette année, N-1, N-2, N-3) ou plage libre du/au, KPI avec variation vs N-1, graphe pleine largeur période courante vs même période N-1 avec granularité jour / semaine / mois selon la longueur. Les fixtures n'ont pas de commandes en 2025 : la ligne N-1 reste à zéro en démo.
- Catalogue refondu le 2026-09-13 : grille de cartes (illustration emoji ou image https, fond orange fruit / vert légume, badges Saison, Bio, drapeau France, Masqué, Indisponible, Stock bas ; prix unitaire et prix au kilo déduit du poids moyen ; contenant, origine, calibre), fiche complète avec aperçu, création via la tuile « Nouveau produit », suppression en deux temps (mot SUPPRIMER exigé aussi côté serveur), visibilité et disponibilité séparées. Parking : téléversement d'images (stockage à définir avec le client), catégories au-delà de fruit / légume si le client en a.
