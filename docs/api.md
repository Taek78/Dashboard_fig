...
- `POST /messages` (avec `Idempotency-Key`) : `subject` parmi `missing_or_damaged`, `delivery_issue`, `order_error`, `product_question`, `refund`, `other` ; `body` (4 000 caractères) ; `orderId` facultatif, qui doit être une commande de la personne (`422 order_not_owned`) ; `attachments` : dix au plus, chacune `{ fileName, contentType, sizeBytes, url }`. Le fichier lui-même est **hébergé par l'application** (question 22) : l'API n'en garde que les métadonnées et l'URL, obligatoirement en `https`. Formats acceptés : PDF, JPEG, PNG, GIF, WebP, AVIF, HEIC, HEIF, TIFF, BMP (ni vidéo ni audio). Réponse `201` (`Message`).
...
3. `POST /service/notifications/{id}/envoi` : accusé d'envoi, qui pose `sentAt` une seule fois (`409 already_sent` ensuite, `404` si inconnue).
