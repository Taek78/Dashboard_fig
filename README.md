...
- [API de l'application](docs/api.md) : comment l'application FIG et son serveur se branchent (accès par code et jeton, catalogue, commandes, messages, notifications) ; description formelle dans [docs/api/openapi.json](docs/api/openapi.json).
...
| `npm run db:demo-accounts`                      | comptes de démonstration livreur et lecture seule (variables `AUTH_DEMO_*` de `.env.local`, base locale) |
...
| Messages        | boîte de réception des demandes « Nous contacter » : recherche, filtres objet, statut, période et importants, épinglés séparés des autres, cartes avec les deux premières lignes et la commande jointe (livraison, préparateur, livreur), dégradé rouge des importants, fiche complète précédée de la fiche client, pièces jointes (photos et PDF) hébergées par le dashboard et affichées dans la fiche, traité / non traité, épingle et drapeau « important »                                                                                                                                                                                                                                         |
...
| Comptes         | gestion des comptes du back-office (administrateur) : création sans mot de passe avec invitation par e-mail (compte en attente d'activation, invitation annulable ; expiration et activation signalées par mail à la personne et aux administrateurs), lien de nouveau mot de passe, désactivation, suppression ; chacun change son mot de passe sur son profil                                                                                                                                                                                                                                                                                                                                         |
| Journal         | journal de sécurité (administrateur) : ce que le back-office enregistre depuis toujours — connexions refusées, accès interdits, changements de rôle, exports RGPD, appels de l'API — en lecture seule, cherchable par adresse, IP ou identifiant, filtrable par famille et par période                                                                                                                                                                                                                                                                                                                                                                                                                  |
| API             | l'application FIG s'y branche ([docs/api.md](docs/api.md)) : accès par code reçu par mail puis jeton de session, inscription, catalogue, profil et autorisations, devis, commandes payées (idempotentes, total vérifié), annulation, messages « Nous contacter », file des notifications pour le serveur de l'application                                                                                                                                                                                                                                                                                                                                                                               |
...
Authentification par e-mail et mot de passe (scrypt), session JWT de 8 heures fermée dès qu'un compte est désactivé, verrouillé ou change de mot de passe, limitation de débit sur la connexion, autorisation vérifiée côté serveur pour chaque écran et chaque écriture, Content-Security-Policy avec nonce, journal de sécurité en base, gardes de démarrage en production, route de santé `/api/health` protégée par `HEALTH_TOKEN` (obligatoire en production) et limitée à une sonde de la base par cinq secondes. CI : actions épinglées par SHA, audit des dépendances, CodeQL et Gitleaks hebdomadaires, accessibilité des écrans vérifiée par axe-core dans la suite navigateur. Comptes créés sans mot de passe (lien d'invitation par e-mail), « Mot de passe oublié » par code à six chiffres valable cinq minutes, « Adresse e-mail oubliée » par le nom du compte, alertes par mail à la personne et aux administrateurs avec un lien « Ce n'était pas moi » qui verrouille le compte, politique de mots de passe (phrase de passe recommandée, mots courants et fuites connues refusés). Mails envoyés par Brevo en production (`MAIL_*` dans `.env.example`), écrits dans `dashboard/.mail/` en développement. Données personnelles : export et anonymisation d'un client, durées de conservation appliquées par script ([docs/rgpd.md](docs/rgpd.md)). L'API de l'application (`/api/v1`) : codes de connexion et jetons de session en HMAC, clé de service, limitation de débit, idempotence des créations, prix recalculés côté serveur, CORS sur liste blanche ([docs/api.md](docs/api.md)). Détail dans [docs/architecture.md](docs/architecture.md#sécurité).
